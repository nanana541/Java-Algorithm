...
## 💡 개선 포인트 및 성능 최적화 (Review)

### 1. 이중 루프 및 불필요한 객체 생성 제거
- **기존 문제점:** `while`문 내부에서 `t.substring()`을 반복 호출하며 문자열을 새로 생성하고 잘라냈습니다. 이는 내부적으로 새로운 메모리 할당과 데이터 복사를 유발하여 문자열이 길어질수록 성능이 급격히 저하됩니다.
- **개선 방법:** 실제로 문자열을 자르는 대신, 변수(카운터) 몇 개만 활용하여 배열의 처음부터 끝까지 단 한 번만 순회하는 **단일 루프($O(N)$)** 방식으로 로직을 단순화했습니다.

### 2. `String.charAt()` 대신 문자형 배열(`char[]`) 활용
- **기존 문제점:** `t.charAt(i)`는 호출될 때마다 자바 내부적으로 인덱스가 범위를 벗어나지 않았는지 검사하는 오버헤드(Bound Check)가 매번 발생합니다.
- **개선 방법:** 시작할 때 `t.toCharArray()`를 통해 `char[]` 배열을 메모리에 한 번만 올려두고 향상된 for문으로 접근했습니다. 연속된 데이터 메모리를 다이렉트로 참조하므로 내부 메서드 호출 비용이 사라져 **실행 속도가 대폭 향상**됩니다.
